...
def _create_temp_pptx(tmp_path: Path) -> Path:
    ...
    # Simple table
    rows, cols = 2, 2
    table_shape = slide.shapes.add_table(rows, cols, Inches(1), Inches(2), Inches(6), Inches(1))
...
async def test_pptx_parser_markdown(tmp_path: Path) -> None:
    ...
    cfg = ParserConfig(output_format="markdown", use_cache=False)
    parser = ParserRegistry.get_parser(pptx_path, cfg)
...
async def test_pptx_parser_json(tmp_path: Path) -> None:
    ...
    cfg = ParserConfig(output_format="json", use_cache=False)
    parser = ParserRegistry.get_parser(pptx_path, cfg)

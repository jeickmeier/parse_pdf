...
def test_registry_list_and_support(tmp_path):
    ...
    assert ParserRegistry.is_supported(txt_file) is True

    # Unsupported extension raises
    unsupported = tmp_path / "file.xyz"
    unsupported.write_text("x")
    with pytest.raises(UnsupportedFormatError):
        ParserRegistry.from_path(unsupported)
...
def test_settings_path_coercion(tmp_path):
    ...
    settings = Settings(cache_dir=str(cache_dir), output_dir=str(out_dir))
...
def test_settings_parser_cfg():
    ...
    settings = Settings(parser_settings=overrides)
